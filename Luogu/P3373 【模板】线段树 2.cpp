...
inline void pushdown(ll s)
{
    ...
    a[ls].w=( (a[ls].w*tagc)%p + ((a[ls].r-a[ls].l+1)%p*tagj)%p )%p;
    a[rs].w=( (a[rs].w*tagc)%p + ((a[rs].r-a[rs].l+1)%p*tagj)%p )%p;
    
    a[ls].tagc=(a[ls].tagc*tagc)%p , a[rs].tagc=(a[rs].tagc*tagc)%p;
...
void change(ll s,ll l,ll r,ll k,ll mode)
{
    ...
    if(l<=mid) change(ls,l,r,k,mode);
    if(r>mid) change(rs,l,r,k,mode);
    a[s].w=(a[ls].w+a[rs].w)%p;
}
ll search(ll s,ll l,ll r)
{
    if(a[s].l>=l && a[s].r<=r)
      return a[s].w;
    pushdown(s);
    ll mid=(a[s].l+a[s].r)>>1,ans=0;
    if(l<=mid) ans=(ans+search(ls,l,r))%p;
